...
- `lidar`: `Bool` (default: `False`). If `True`, the robot also has the front head link and the `lidar_link` sensor frame (Ayg 1.3 replaced the D555 depth camera with a HESAI JT128).
...
```shell
ros2 run xacro xacro urdf/ayg.xacro sim:=False lidar:=False > urdf/ayg.urdf
```
